...
class CameraApp:
    def __init__(self, root):
        ...
        with open('cube_data.json') as f:
            data = json.load(f)
        self.cube_data = [tuple(tuple(point) for point in face) for face in data]
        ...
        for face in self.cube_data:
            ...
            for point in face:
                ...
                if z_rot <= 0:
                    ...
                    break
                depths.append(z_rot)
                x_proj = (x_rot / z_rot) * self.focal_length
                y_proj = (y_rot / z_rot) * self.focal_length
                screen_x = 400 + x_proj
                screen_y = 300 - y_proj
                points.append((screen_x, screen_y))
            if not skip:
                avg_depth = sum(depths) / len(depths)
                faces_with_depth.append((avg_depth, points))
        # Sort by depth (descending)
        faces_with_depth.sort(key=lambda x: -x[0])
        for depth, points in faces_with_depth:
