...
###############################################################################
def project_point(point3d, camera, canvas_w, canvas_h):
    """
...
def render_bsp(root, camera, canvas):
    ...
    if is_in_front_of_plane(camera.pos, root.plane_normal, root.plane_point):
        ...
        render_bsp(root.back, camera, canvas)
        draw_polygon(root.polygon, camera, canvas)
        render_bsp(root.front, camera, canvas)
    else:
...
def draw_polygon(polygon, camera, canvas):
    """
...
class Camera:
    ...
    def get_forward(self):
        ...
        NOTE: In this sample, forward is determined by:
          ...
          fy = cos(pitch)*(-cos(yaw))    (the minus sign means +yaw turns left)
          fz = sin(pitch)
        """
    ...
    def get_up_direction(self):
        """
        If we wanted a true camera 'up', we could do cross(right, forward),
        but for vertical movement let's keep it simple as world up (0,0,1).
        """
        return (0, 0, 1)


###############################################################################
#                  Tkinter App with Extended Movement Keys
###############################################################################
class App:
