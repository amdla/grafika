...
class Material:
    def __init__(self, name, ambient, diffuse, specular, shininess, color):
        self.name = name
        self.ka = ambient  # Współczynnik ambient (otoczenia)
        self.kd = diffuse  # Współczynnik diffuse (rozproszenia)
        self.ks = specular  # Współczynnik specular (kierunkowy)
        self.shininess = shininess  # Połysk (wyższy = ostrzejsze odbicie)
        self.color = color  # Kolor materiału


# Materiały predefiniowane
MATERIALS = {
    ...
    "dobry_plastik": Material(
        "Dobry Plastik",
        ambient=0.1,
        diffuse=0.6,  # Zwiększony współczynnik diffuse
        specular=0.7,
        shininess=30,  # Nieco zmniejszony połysk
        color=Color(0.2, 0.5, 0.7)
    ),
    "zloto": Material(
        "Złoto",
        ambient=0.3,
        diffuse=0.4,  # Nieco zwiększony współczynnik diffuse
        specular=1.0,
        shininess=128,  # Zwiększony połysk dla wyraźniejszego odbicia
...
def phong_reflection(point, normal, view_dir, material, lights):
    ...
    ambient = material.color.multiply(material.ka)
    ...
    for light in lights:
        ...
        if dot_product > 0:
            diffuse_intensity = dot_product * material.kd * light.intensity
            diffuse = material.color.multiply(diffuse_intensity)
        ...
        if spec_angle > 0:
            spec_intensity = pow(spec_angle, material.shininess) * material.ks * light.intensity
            specular = light.color.multiply(spec_intensity)
